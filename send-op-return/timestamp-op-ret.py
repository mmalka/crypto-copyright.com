...
print("-----------------------------+=======================================================================+---------------------------")
print("-----------------------------+ OP_RETURN transaction builder by crypto-copyright.com, copyright 2014 +---------------------------");
print("-----------------------------+  Builds raw transaction, signs it, broadcasts it to bitcoin network.  +---------------------------")
print("-----------------------------+             usage: script.py [\"valid-sha3/224-hash1\",...]             +---------------------------");
print("-----------------------------+=======================================================================+---------------------------")
...
for myHash in sys.argv[1:]:
    try:
        ...
        if len(myHash) > 56:
            ...
            print("Size: " + str(len(myHash)) + ", input: " + str(myHash))
            exit(0)
        print("The input hash is a valid SHA-3/224 hash | Size: " + str(len(myHash)) + " | input: " + str(myHash))
...
# We have a valid input, let's append our marker to it.
        ...
        continue
print("---------------------------------------------------------------------------------------------------------------------------------")
print("Processing the inputs... " + str(len(digests)) + " valids SHA-3/224 hashs found.")
print("---------------------------------------------------------------------------------------------------------------------------------")
i = 1;
for digest in digests:
    print("Processing #" + str(i) + " OP_RETURN | Size: " + str(len(digest)) + " | Value: " + str(digest))
    print("---------------------------------------------------------------------------------------------------------------------------------")
    i += 1
    txouts = []
    ...
    while True:
        ...
        if value_in - tx.vout[1].nValue >= len(tx.serialize())*FEE_PER_BYTE:
            ...
            print("Generated TxRaw | Size: " + str(size_txraw) + " | Value: " + content_txraw)
            print("Transaction Broadcast -> Generated TxId | " + b2lx(proxy.sendrawtransaction(tx)))
            print("---------------------------------------------------------------------------------------------------------------------------------")
            break
